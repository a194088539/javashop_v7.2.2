      ...
      <div v-for="(item, $index) in skuInfo" :key="$index">
        <el-form-item label="规格名：">
          <el-autocomplete
            ...
            @blur.naitve="editSkuItems(item, $index)"
  ...
  export default {
    ...
    methods: {
      ...
      handleCloseSkuItem($index) {
        if (this.checkedImage && $index === 0) {
          this.checkedImage = false
        }
        this.skuInfo.splice($index, 1)
        /**
         * @updata
         * @Date 2020/4/30 上午10:45:23
         * 此处修改目的：防止影响当前页面的变量信息
         * */
        this.$emit('updateSkuInfo', cloneObj(this.skuInfo))
      },
      ...
      handleSelectSkuItem(item) {
        if (item.value === '' || item.spec_name === undefined) {
          return
        }
        /** 当前规格项是否为选择项 */
        ...
        const _skuInfo = this.skuInfo.filter((key) => {
          if (item.spec_id === undefined) return
      ...
      editSkuItem(item, $index) {
        ...
        setTimeout(() => {
          ...
          if (item.spec_name === '' || item.spec_name === undefined) {
            return
          }
          /** 当前规格项是否为选择项 */
          if (this.activeSkuItem.spec_name === item.spec_name) {
            return
          }
          /** 判断规格项的值是否已选择 */
          ...
          const _isexit = this.skuData.filter(key => {
            if (item.spec_name === undefined) return
            return key.spec_name === item.spec_name
          })
          /** 如果已经存在则提示已存在 不允许添加 */
          if (_isexit.length >= 1) {
      ...
      /** 添加当前规格项的规格值*/
      addSpec(index, item) {
        if (this.skuInfo[index].spec_name === '') {
      ...
      handleSelectSkuValue(val) {
        ...
        if (_value_list.length > 1) {
          ...
          } else if (this.skuInfo[this.activeSkuItemIndex].value_list[this.activeSkuValIndex].spec_value_id) {
            /**
             * @updata
             * @Date 2020/4/29 下午9:56:41
             * 此处修改目的：如果选择了已经选择过的规格，则不允许其修改，恢复成原选择项
             * **/
            this.$set(this.skuInfo[this.activeSkuItemIndex].value_list[this.activeSkuValIndex], 'spec_id', this.activeSkuVal.spec_id)
            this.$set(this.skuInfo[this.activeSkuItemIndex].value_list[this.activeSkuValIndex], 'spec_name', this.activeSkuVal.spec_name)
      ...
      editSkuIValue(item, val, $index, index) {
        /** 当前规格项是否为选择项 */
        if (this.activeSkuVal.spec_value === val.spec_value) {
          return
        }
        /** 检测是否已选*/
        const _value_list = this.skuInfo[this.activeSkuItemIndex].value_list.filter((key) => {
        ...
        if (_isexit.length >= 1) {
          ...
          if (!this.skuInfo[this.activeSkuItemIndex].value_list[this.activeSkuValIndex].spec_value_id) {
             ...
             * 此处修改目的：如果选择了已经选择过的规格，则不允许其修改，恢复成原选择项
             * @desc
             *    防止规格值正在修改中，同时点击了添加规格值，之前修改的规格值错乱问题
             * **/
            this.$set(this.skuInfo[this.activeSkuItemIndex].value_list[this.activeSkuValIndex], 'spec_id', this.activeSkuVal.spec_id)
            this.$set(this.skuInfo[this.activeSkuItemIndex].value_list[this.activeSkuValIndex], 'spec_name', this.activeSkuVal.spec_name)
            this.$set(this.skuInfo[this.activeSkuItemIndex].value_list[this.activeSkuValIndex], 'spec_value', this.activeSkuVal.spec_value)
