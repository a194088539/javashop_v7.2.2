...
<script>
  ...
  export default {
      ...
      updateSkuInfo(target) {
        ...
        if (this.value_list.length > 0) {
          for (let i in this.skuInfo) {
            for (let s in this.value_list) {
              if (this.skuInfo[i].spec_value_id === this.value_list[s].spec_value_id) {
                this.$set(this.skuInfo[i], 'cost', this.value_list[s].cost)
                this.$set(this.skuInfo[i], 'price', this.value_list[s].price)
                this.$set(this.skuInfo[i], 'sn', this.value_list[s].sn)
                this.$set(this.skuInfo[i], 'weight', this.value_list[s].weight)
                this.$set(this.skuInfo[i], 'quantity', 0)
              }
            }
          }
        }
    ...
    mounted() {
      this.$bus.$on('listenToA', this.list)
    }
  }
</script>
