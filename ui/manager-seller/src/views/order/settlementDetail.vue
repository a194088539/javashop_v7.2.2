<template>
  <div class="bg-settlement">
    ...
    <div class="sum-settlement">
      ...
      <p>
        <span>分销退还佣金总金额：</span>
        <span> {{ settlementData.distribution_return_rebate | unitPrice('￥') }} </span>
      </p>
      <p>
        <span>本期应收金额：</span>
        <span><strong style="color: red;font-size: 18px;">{{ settlementData.bill_price | unitPrice('￥') }}</strong> = {{ settlementData.price | unitPrice('￥') }} (在线支付订单收入总金额) - {{ settlementData.refund_price | unitPrice('￥') }} (在线支付订单退款总金额) -
          {{ settlementData.commi_price | unitPrice('￥') }} (平台收取佣金总金额) - {{ settlementData.distribution_rebate | unitPrice('￥') }} (分销支付佣金总金额) +
          {{ settlementData.refund_commi_price | unitPrice('￥') }} (平台退还佣金总金额) + {{ settlementData.site_coupon_commi | unitPrice('￥') }} (用户使用平台优惠券总金额) +
          {{ settlementData.distribution_return_rebate | unitPrice('￥') }} (分销退还佣金总金额)</span>
      </p>
      <p>
        <span>结算状态：</span>
      ...
      exportExcel() {
        API_Settlement.exportSettleMentExcel(this.billId).then(response => {
          ...
          jsonArray[0] = {
            ...
            sheet_options: { header: ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L'], skipHeader: true }
          }
          jsonArray[1] = {
            sheet_name: '订单列表',
