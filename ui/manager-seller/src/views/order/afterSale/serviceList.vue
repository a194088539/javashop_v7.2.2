        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-if="!scope.row.allowable.allow_put_in_store && !scope.row.allowable.allow_audit && !scope.row.allowable.allow_seller_refund && !scope.row.allowable.allow_seller_create_order">
              详情</el-button>
          </template>
        </el-table-column>
      </template>
  ...
  export default {
    ...
    methods: {
      ...
      handleServiceDetail(row) {
        this.$router.push({ path: `/order/after-sale/service-detail/${row.service_sn}` })
      },
