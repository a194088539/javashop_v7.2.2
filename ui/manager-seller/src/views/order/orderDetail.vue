<template>
  <div v-loading="loading" class="order-detail-container">
    ...
    <div class="order-info" v-if="orderDetail">
      ...
      <el-collapse  class="order-collapse" :value="['order', 'promotions', 'other']">
        ...
        <el-collapse-item title="订单信息" name="order">
          ...
          <div class="order-item">
            ...
            <span class="item-value">{{ orderDetail.ship_mobile | secrecyMobile }}</span>
          </div>
          <hr>
          <!--订单编号 付款方式 下单时间-->
          <div class="order-item">
          ...
          <div class="order-item">
            ...
            <span class="item-value">{{ orderDetail.goods_original_total_price | unitPrice('¥') }}</span>
          </div>
          <div class="order-item">
        ...
        <el-collapse-item title="其他信息（发票、备注）" name="other">
          ...
          <template v-if="orderDetail.receipt_history">
            ...
            <div class="order-item">
              ...
              <span class="item-value"><a href="#" @click="toReceiptDetail(orderDetail.receipt_history.history_id)" >查看详情</a></span>
            </div>
          </template>
        </el-collapse-item>
      </el-collapse>
      ...
      <div class="order-status-info">
        ...
        <div v-if="logisticsStatus">
          ...
          <div class="order-item" v-if="orderDetail.order_operate_allowable_vo.allow_check_cancel && orderDetail.order_status != 'CANCELLED'">
        ...
        <div class="opera-btn">
          <el-button plain type="primary" @click="confirmInvoice">打印发货单</el-button>
          <el-button v-if="isShowEditShipName" plain type="primary" @click="adjustConsignee" >修改收货人信息</el-button>
          <el-button v-if="isShowEditOrderPrice" plain type="primary" @click="adjustPrice" >调整价格</el-button>
          <el-button v-if="isShowConfirmReceive" plain type="primary" @click="confirmReceive" >确认收款</el-button>
          <el-button v-if="isShowCancelOrder && orderDetail.payment_type === 'COD'" plain type="primary" @click="cancelDialogShow = true" >取消订单</el-button>
        </div>
      </div>
    </div>
    <!--订单状态 步骤条-->
    ...
    <div>
      <el-table :data="productList" :header-cell-style="{textAlign: 'center'}">
        ...
        <el-table-column label="商品名称">
          <template slot-scope="scope">
            <a :href="`${MixinBuyerDomain}/goods/${scope.row.goods_id}`" target="_blank" style="color: #00a2d4;" v-html="scope.row.name">{{ scope.row.name }}</a>
      ...
      <div align="center">
        ...
        <el-form
          ...
          <el-form-item prop="adjustedPrice">
            <el-input placeholder="请输入订单总价" v-model="adjustedPrice" @change="() => { adjustedPrice = adjustedPrice.trim() }" :maxlength="10">
    ...
    <!--电子面单-->
    <el-dialog title="电子面单" :visible.sync="electronicSurfaceShow" width="32%" align="center">
      <!--主体-->
    ...
    <!--发货单-->
    <el-dialog title="发货单" :visible.sync="invoiceShow">
      <div>
        <el-button type="primary" @click="handlePrintInvoice">打印</el-button>
      </div>
      <ul class="invoice" ref="deliverySheet" id="deliverySheet">
          <li class="site">
            <div>
              <img :src="invoiceDetail.logo"/>
            </div>
            <div>
              <span>{{ invoiceDetail.site_name }}</span>
              <span>{{ invoiceDetail.site_address }}</span>
            </div>
            <div>
              <span>收货人：{{ invoiceDetail.consignee }}</span>
              <span>会员：{{ invoiceDetail.member_name }}</span>
            </div>
          </li>
          <li class="order">
          ...
          <li class="table">
            <el-table :data="invoiceDetail.order_sku_list">
          ...
          <li class="receiving">
            <div>
              <span>收货人：{{ invoiceDetail.consignee }}</span>
              <span>地址：{{ invoiceDetail.region }} {{ invoiceDetail.address }}</span>
              <span>电话：{{ invoiceDetail.phone || ''  | secrecyMobile}}</span>
            </div>
          </li>
      </ul>
    </el-dialog>
        ...
        <el-form :model="cancelOrderForm" :rules="cancelOrderRules" ref="cancelOrderForm">
          <el-form-item prop="reason">
            <el-input
              ...
              placeholder="请输入取消原因~(3-120字之间)"
              :maxlength="120"
              v-model="cancelOrderForm.reason">
            </el-input>
          </el-form-item>
        </el-form>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      toReceiptDetail(history_id) {
        this.$router.push({ path: '/order/receipt-detail/' + history_id })
      },
      ...
      deliverGoods(row) {
        ...
        const _params = {
          ...
          ship_no: row.ship_no,
          /** 物流公司id */
          logi_id: row.logistics_company_do.id,
          /** 物流公司名称 */
          logi_name: row.logistics_company_do.name
        }
...
<style type="text/scss" lang="scss" scoped>
  ...
  /deep/ .el-table .el-table__body-wrapper tbody tr:last-child {
    td {
      border-bottom: none;
    }
  }
