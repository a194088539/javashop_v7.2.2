          ...
          <el-form
            ...
            <div class="base-info-item">
              ...
              <div>
                ...
                <!--自营店--送积分-->
                <el-form-item prop="point_value" class="discount-model" v-if="parseInt(shopInfo.self_operated) === 1">
                  ...
                  <span class="integral-show" v-show="isSendPoint">
                    <el-input  size="mini" v-model="activityForm.point_value" :maxlength="8"></el-input> 分
                  </span>
                </el-form-item>
                <!--免邮费-->
                ...
                <el-form-item prop="bonus_id" class="discount-model">
                  <el-checkbox :label="couponTxt" v-model="is_send_bonus" @change="isChangeCoupon"></el-checkbox>
	                <el-select v-if="isExitcanChooseCoupons" v-show="is_send_bonus" v-model="activityForm.bonus_id" placeholder="请选择" @change="changeCoupon">
		                <el-option
                  ...
                  <template v-else-if="$status$ === 'UNDERWAY'">
                    <el-select v-show="is_send_bonus" :value="couponList.length && couponList.find(i => i.coupon_id == activityForm.bonus_id) && couponList.find(i => i.coupon_id == activityForm.bonus_id).title" placeholder="请选择"></el-select>
                  </template>
	                <el-select v-else v-show="is_send_bonus" value="优惠券已失效" placeholder="请选择"></el-select>
                              ...
                              <div class="goods-info">
                                ...
                                <div>
                                  <a :href="`${MixinBuyerDomain}/goods/${scope.row.goods_id}`" target="_blank"
                                    style="color: #00a2d4;" v-html="scope.row.goods_name">{{ scope.row.goods_name }}</a>
...
<script>
  ...
  import { CategoryPicker, GoodsSkuPicker, UE } from '@/components'
  ...
  export default {
    ...
    computed: {
      ...
      /** 是否存在匹配的优惠券选项 */
      isExitcanChooseCoupons() {
        if (!this.editEnabled) { // 如果是查看状态
          if (this.couponList && this.couponList.length) {
            const _coupon = this.couponList.find(key => key.bonus_id === this.activityForm.bonus_id)
            return !!_coupon
          } else {
            return false
          }
    ...
    data() {
      ...
      const checkIntegral = (rule, value, callback) => {
        if (this.isIntegral) {
          ...
          if (this.activityForm.is_send_point) {
            if (!this.activityForm.point_value) {
              return callback(new Error('请输入积分值'))
            } else if (String(this.activityForm.point_value).length > 8) {
              return callback(new Error('输入长度不能超过8位'))
            }
            setTimeout(() => {
              if (!RegExp.integer.test(this.activityForm.point_value)) {
                callback(new Error('请输入数字值'))
              } else {
                callback()
              }
    ...
    methods: {
      ...
      handleEditMould(row) {
        this.activeName = 'add'
        this.fullCutName = (row.status === 'END' || row.status === 'UNDERWAY') ? '查看满优惠' : '编辑满优惠'
        this.editEnabled = !(row.status === 'END' || row.status === 'UNDERWAY')
        this.$status$ = row.status
        setTimeout(() => { this.$refs['activityForm'].resetFields() })
        this.GET_FullCutActivityDetails(row.fd_id, row)
      },
      ...
      /** 获取单个商品详情 */
      GET_FullCutActivityDetails(id, row) {
