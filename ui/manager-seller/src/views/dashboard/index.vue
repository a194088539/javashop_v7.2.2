        ...
        <el-card class="box-card">
          ...
          <div class="store-index-content">
            ...
            <div>
              ...
              <el-tag type="success" @click.native="toRefundOrderList()">     待处理售后 {{dashBoardData.after_sale_order_num}}</el-tag>
            </div>
          </div>
        </el-card>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    toConsumerMsg() {
      const result = this.$store.getters.addRouters.some(key => {
        if (key.name === 'customer' && key.children) {
          return key.children.some(item => item.name === 'consultation')
        }
        return false
      })
      if (result) {
    ...
    toRefundOrderList() {
      const result = this.$store.getters.addRouters.some(key => {
        if (key.name === 'order' && key.children) {
          return key.children.some(item => item.name === 'afterSale')
        }
        return false
      })
      if (result) {
        this.$router.push({ path: '/order/after-sale/service-list' })
      } else {
        this.$message.error('权限不足，无法查看')
      }
    },
