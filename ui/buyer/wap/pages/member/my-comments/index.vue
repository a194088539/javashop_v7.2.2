      ...
      <van-list
        ...
        <div class="order-item" v-for="(order, index) in orderList" :key="index">
          ...
          <div class="info-order-item">
            ...
            <div class="order-btns" v-if="order.comment_status === 'UNFINISHED'" style="top: 17px;">
              <nuxt-link :to="'/member/comments?order_sn=' + order.sn">去评论</nuxt-link>
            </div>
            <div class="order-btns" v-if="order.audit_status === 'PASS_AUDIT' && params.order_status === 'WAIT_CHASE'" style="top: 7px;">
              <nuxt-link :to="`/member/comments?order_sn=${order.order_sn}&sku_id=${order.sku_id}&append_comment=review`">追加评论</nuxt-link>
            </div>
            <div class="order-btns" v-if="params.order_status === 'FINISHED'" style="top: 7px;">
              <nuxt-link :to="`/member/my-comments/detail?comment_id=${order.comment_id}`">查看评价</nuxt-link>
            </div>
          </div>
          <nuxt-link :to="'/shop/' + order.seller_id" class="shop-order-item" v-if="params.order_status === 'WAIT_COMMENT'">
          ...
          <div v-if="params.order_status !== 'WAIT_COMMENT'" class="sku-order-item">
            <div class="sku-content">
              <nuxt-link :to="'/goods/' + order.goods_id" class="img-info">
                <img :src="order.goods_img" :alt="order.goods_name">
              </nuxt-link>
              <nuxt-link :to="`/goods/${order.goods_id}`" style="margin-top: 10px">
                <div class="sku-name" v-html="order.goods_name">{{ order.goods_name }}</div>
              </nuxt-link>
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      handleTabChange(index) {
        ...
        if (this.params.order_status === 'WAIT_CHASE') {
