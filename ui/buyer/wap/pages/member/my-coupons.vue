      ...
      >
        <div class="coupon-item content-unavailable show" v-for="(coupon, index) in couponsList" :key="index" :class="[coupon.used_status === 1 && 'used', coupon.used_status === 2 && 'overdue']">
          <div class="inner-coupon" :class="[tabActive === 1 && 'unavailable']">
            <div class="par">
              <p v-if="coupon.order_sn" style="padding-bottom: 10px;font-weight: bold;">订单号：{{coupon.order_sn}}</p>
              <p>{{ coupon.title }}</p>
              <sub class="sign">￥</sub><span>{{ coupon.coupon_price | unitPrice }}</span>
              <p>订单满{{ coupon.coupon_threshold_price | unitPrice }}元</p>
              <p style="padding-top:10px;" v-if="coupon.use_scope">
                仅限
              ...
              <p style="padding-top:10px;" v-else>仅限 <nuxt-link :to="'/shop/' + coupon.seller_id" style="color: #fff">{{ coupon.seller_name }}</nuxt-link> 店铺使用</p>
