  ...
  <div id="group-buy">
    ...
    >
      <li v-for="(goods, index) in assemble" :key="index" class="item-goods">
        <dt>
	        <nuxt-link v-if="!goods.is_enable_sale">
		        <img :src="goods.thumbnail" :alt="goods.goods_name">
	        </nuxt-link>
          <nuxt-link v-else :to=" `/goods/${goods.goods_id}?sku_id=${goods.sku_id}&pintuan_id=${goods.pin_tuan_id}&from_nav=assemble`">
            <img :src="goods.thumbnail" :alt="goods.goods_name">
          </nuxt-link>
        </dt>
        <dd>
	        <nuxt-link v-if="!goods.is_enable_sale">
		        <div class="name-goods" v-html="goods.goods_name">{{ goods.goods_name }}</div>
	        </nuxt-link>
          <nuxt-link v-else :to="`/goods/${goods.goods_id}?sku_id=${goods.sku_id}&pintuan_id=${goods.pin_tuan_id}&from_nav=assemble`">
            <div class="name-goods" v-html="goods.goods_name">{{ goods.goods_name }}</div>
          </nuxt-link>
          ...
          <div class="descrip-goods">
            ...
            <nuxt-link v-if="!goods.is_enable_sale" class="grab-btn">已售空</nuxt-link>
            <nuxt-link v-else :to="`/goods/${goods.goods_id}?sku_id=${goods.sku_id}&pintuan_id=${goods.pin_tuan_id}&from_nav=assemble`" class="grab-btn">去开团</nuxt-link>
          </div>
        </dd>
      </li>
    </van-list>
