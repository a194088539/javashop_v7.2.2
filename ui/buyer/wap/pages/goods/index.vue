    ...
    <van-list v-model="loading" :finished="finished" @load="onLoad" class="goods-list">
      <nuxt-link :to="'/goods/' + goods.goods_id" v-for="(goods, index) in goodsList" :key="index" class="goods-item">
        ...
        <div class="goods-info">
          <div class="goods-name" v-html="goods.name">{{ goods.name }}</div>
  ...
  export default {
    ...
    beforeRouteLeave(to, from, next) {
      if (to.name === 'goods-id'){
        ...
        localStorage.setItem('SAVE_POSITION', position)
      } else {
        let position = window.scrollY
    ...
    methods: {
      goBack() {
        if (this.fromRoute.name === 'category') {
          return this.$router.push('/category?catId=' + this.$route.query.catId)
        } else {
