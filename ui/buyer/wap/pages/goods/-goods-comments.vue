  ...
  <div id="goods-comments" class="goods-comments">
    <van-cell-group class="comment-cells">
      ...
      <div v-else class="some-comments">
       <!--初评信息-->
        <template v-for="(comment, index) in comments" v-if="comment.audit_status === 'PASS_AUDIT'">
          <div v-if="index < 3" :key="index" class="com-item">
            ...
            <div class="com-content comment-content">{{ comment.content }}</div>
            ...
            <div v-if="comment.reply_status === 1 && comment.reply" class="com-reply">
              <span>掌柜回复：</span>
              <span class="reply-content comment-content">{{ comment.reply.content }}</span>
            </div>
            <!--追评信息-->
            <div class="append" v-if="comment.additional_comment && comment.additional_comment.audit_status === 'PASS_AUDIT'">
              <p>追加评论 {{comment.additional_comment.create_time | unixToDate('yyyy-MM-dd')}}</p>
              <div class="com-content comment-content">{{ comment.additional_comment.content }}</div>
              <div v-if="comment.additional_comment.images && comment.additional_comment.images.length" class="com-gallery">
                  <span v-for="(img, index) in comment.additional_comment.images" :key="index" class="img">
                    <img :src="img" @click="handleImagePreview(comment.additional_comment.images, index)">
                  </span>
              </div>
              <div v-if="comment.additional_comment.reply_status === 1 && comment.additional_comment.reply" class="com-reply">
                <span>掌柜回复：</span>
                <span class="reply-content comment-content">{{ comment.additional_comment.reply.content }}</span>
              </div>
            </div>
          </div>
        </template>
    ...
    <van-popup v-model="showPopup" position="bottom" style="height:100%">
      ...
      <van-list
        ...
        class="comments-list"
      >
        <div v-for="(comment, index) in comments" :key="index" class="com-item" :class="[index === comments.length - 1 && 'last']" v-if="comment.audit_status === 'PASS_AUDIT'">
          <div class="com-user">
            <span class="user">{{ comment.member_name }}</span>
            <span class="credit">{{ comment.grade | filterGrade }}</span>
            <span class="date">{{ comment.create_time | unixToDate('yyyy-MM-dd') }}</span>
          ...
          <div v-if="comment.reply_status === 1 && comment.reply" class="com-reply">
            <span>商家回复：</span>
            <span class="reply-content">{{ comment.reply.content }}</span>
          </div>
          <!--追评信息-->
          <div class="append" v-if="comment.additional_comment && comment.additional_comment.audit_status === 'PASS_AUDIT'">
            <p>追加评论 {{comment.additional_comment.create_time | unixToDate('yyyy-MM-dd')}}</p>
            <div class="com-content">{{ comment.additional_comment.content }}</div>
            <div v-if="comment.additional_comment.images && comment.additional_comment.images.length" class="com-gallery">
                  <span v-for="(img, index) in comment.additional_comment.images" :key="index" class="img">
                    <img :src="img" @click="handleImagePreview(comment.additional_comment.images, index)">
                  </span>
            ...
            <div v-if="comment.additional_comment.reply_status === 1 && comment.additional_comment.reply" class="com-reply">
              <span>商家回复：</span>
              <span class="reply-content">{{ comment.additional_comment.reply.content }}</span>
            </div>
          </div>
        </div>
      </van-list>
