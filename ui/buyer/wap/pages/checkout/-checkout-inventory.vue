    ...
    <van-cell-group v-for="(shop, shopIndex) in inventories" :key="'shop_' + shopIndex">
      ...
      <van-cell class="shop-inner-cell">
        <div>
          ...
          总重量：<span class="price">{{ shop.weight }}kg</span>
        </div>
      </van-cell>
      <van-cell v-if="shop.promotion_notice && shop.showPromotionNotice" class="shop-inner-cell">
      ...
      <van-cell v-for="(sku, skuIndex) in shop.sku_list" :key="'sku_' + skuIndex">
        <div class="sku-item">
          ...
          <div class="sku-info">
            <strong class="sku-name" v-html="sku.name">{{ sku.name }}</strong>
      ...
      <van-cell-group :border="false">
        <van-cell v-for="(coupon, couponIndex) in surActsShop.gift_coupon_list" :key="'coupon_' + couponIndex">
          <div class="popup-act-item">
            ...
            <div class="popup-act-name">
              价值<span class="price">
              {{ coupon.amount | unitPrice }}元</span>
              的优惠券
              [{{ coupon.use_term }}]
            </div>
          </div>
        ...
        <van-cell v-for="(gift, giftIndex) in surActsShop.gift_list" :key="'gift_' + giftIndex">
          <div class="popup-act-item">
            ...
            <div class="popup-act-name">
              价值<span class="price">
              {{ gift.gift_price | unitPrice }}元</span>
              的{{ gift.gift_name }}
            </div>
          </div>
        </van-cell>
