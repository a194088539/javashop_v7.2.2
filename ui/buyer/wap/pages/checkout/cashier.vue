<template>
  <div id="cashier">
    <van-nav-bar title="订单创建成功"/>
    <div v-if="order" class="success-item">
      <div class="inner-delivery">
        <img src="data:image/png;base64," alt="">
        <div class="content-delivery">
          <p>支付方式：<span style="color: #ff002d;">{{ order.pay_type_text === 'ONLINE' ? '在线支付' : '货到付款' }}</span></p>
          <p>订单金额：<span style="color: #ff002d;">￥{{ order.need_pay_price | unitPrice }}</span></p>
        </div>
      </div>
      <div class="btns-delivery">
        <nuxt-link to="/member/my-order" replace class="btns-item">
          <button type="button">查看订单</button>
        </nuxt-link>
        <nuxt-link to="/" replace class="btns-item">
          <button type="button">返回首页</button>
        </nuxt-link>
      </div>

      <!-- 预存款支付 -->
      <div class="payment-pre-deposit" v-if="order.pay_type_text === 'ONLINE' && order.need_pay_price !== 0 && order.pay_status !== 'PAID_OFF' && order.pay_status !== 'PAY_YES'">
        <van-checkbox
          v-model="checked"
          :disabled="!(account_balance > 0)"
          shape="square">
          使用预存款支付（预存款余额￥{{account_balance | unitPrice}}，目前还需在线支付￥
          <template>
            {{ order.need_pay_price | unitPrice}}
          </template>
          ）余额不足？<nuxt-link to="/member/account-balance?type=recharge" style="color: #333;" @click="e => e.stopPropagation()">马上充值</nuxt-link>
        </van-checkbox>
        <!-- 密码输入框 -->
        <div v-if="checked">
          <van-password-input
            :value="password"
            @focus="showKeyboard = true"
          />
          <div class="tip">
            <van-icon name="fail" /> <span>支付密码与用户登录密码不同，可在用户中心进行设置</span>
            <nuxt-link to="/member/account-safe" v-if="!order.deposite.is_pwd">您还未设置支付密码?</nuxt-link>
            <nuxt-link to="/member/account-safe" v-else>忘记密码?</nuxt-link>
          </div>
        </div>
        <!-- 数字键盘 -->
        <van-number-keyboard
          :show="showKeyboard"
          @input="onInput"
          @delete="onDelete"
          @blur="showKeyboard = false"
        />
      </div>

      <div v-if="order.pay_type_text === 'ONLINE' && order.need_pay_price !== 0 && (order.pay_status !== 'PAID_OFF' && order.pay_status !== 'PAY_YES')" class="success-item" style="margin-top: 20px">
        <van-cell-group>
          <van-cell
            v-for="payment in paymentList"
            :key="payment.plugin_id"
            :value="payment.method_name"
            is-link
            @click="initiatePay(payment, 'normal')"
          >
            <img :src="payment.image" slot="title" class="img-payment">
          </van-cell>
        </van-cell-group>
      </div>
      <div class="__pay_form__"></div>
    </div>
  </div>
</template>

<script>
  import * as API_Trade from '@/api/trade'
  import * as API_Deposit from '@/api/deposit'
  import request from '@/utils/request'
  export default {
    name: 'cashier',
    middleware: 'auth-user',
    head() {
      return {
        title: `订单支付-${this.site.title}`
      }
    },
    data() {
      return {
        trade_sn: this.$route.query.trade_sn,
        order_sn: this.$route.query.order_sn,
        // 支付方式列表
        paymentList: [],
        // 订单详情
        order: '',
        // 支付方式
        payment_plugin_id: '',
        // 显示确认订单完成支付dialog
        showConfirmDialog: false,
        // 预存款余额
        account_balance: 0,
        // 是否使用预存款支付
        checked: false,
        // 数字键盘是否打开
        showKeyboard: false,
        // 预存款密码
        password: ''
      }
    },
    mounted() {
      Promise.all([
        this.trade_sn
          ? API_Trade.getCashierData({ trade_sn: this.trade_sn })
          : API_Trade.getCashierData({ order_sn: this.order_sn }),
        API_Trade.getPaymentList('WAP'),
        API_Deposit.getDepositBalance()
      ]).then(responses => {
        this.order = responses[0]
        this.account_balance = responses[2]
        let paymentList = responses[1]

        // 如果是在微信里，隐藏支付宝支付方式
        if (this.MixinIsWeChatBrowser()) {
          paymentList = paymentList.filter(item => item.plugin_id !== 'alipayDirectPlugin')
        }
        // else {
        //   paymentList = paymentList.filter(item => item.plugin_id !== 'weixinPayPlugin')
        // }
        this.paymentList = paymentList.map(item => {
          item.selected = false
          return item
        })
      })
    },
    methods: {
      onInput(key) {
        this.password = (this.password + key).slice(0, 6);
        if (this.password.length === 6) {
          this.showKeyboard = false
          const trade_type = this.trade_sn ? 'TRADE' : 'ORDER'
          const sn = this.trade_sn || this.order_sn
          const params = {
            sn,
            trade_type,
            password: this.password
          }
          API_Deposit.getBalancePay(trade_type, sn, params).then(response => {
            if(response.need_pay === 0) {
              this.$router.replace('/payment-complete')
            } else {
              location.reload()
            }
          })
        }
      },
      onDelete() {
        this.password = this.password.slice(0, this.password.length - 1);
      },
      /** 发起支付 */
      initiatePay(payment, pay_mode) {
        this.payment_plugin_id = payment.plugin_id
        const trade_type = this.trade_sn ? 'trade' : 'order'
        const sn = this.trade_sn || this.order_sn
        const client_type = 'WAP'
        const payment_plugin_id = payment.plugin_id
        API_Trade.initiatePay(trade_type, sn, {
          client_type,
          pay_mode: 'normal',
          payment_plugin_id
        }).then(response => {
          //如果在在微信浏览器内
          if (this.MixinIsWeChatBrowser()) {
            const params = {}
            if (response.form_items) {
              response.form_items.forEach(item => {
                params[item.item_name] = item.item_value
              })
            } else {
              Object.keys(response).forEach(key => {
                if (key !== 'gateway_url') {
                  params[key] = response[key]
                }
              })
            }
            WeixinJSBridge.invoke('getBrandWCPayRequest', params, this.payCallbackDialog)
          } else {
            let $a = `<a id="cashierLoad" href="./cashier-load-pay?trade_type=${trade_type}&sn=${sn}&payment_plugin_id=${payment_plugin_id}" target="_blank"></a>`;
            document.getElementsByClassName('__pay_form__')[0].innerHTML = $a
            document.getElementById("cashierLoad").click()
            this.payCallbackDialog()
          }
        })
      },
      payCallbackDialog() {
        this.$dialog.confirm({
          title: '提示',
          message: '请确认支付是否完成！',
          confirmButtonText: '支付成功',
          cancelButtonText: '重新支付'
        }).then(() => {
          this.$router.replace({ path: '/member/my-order' })
        }).catch(() => {})
      }
    }
  }
</script>

<style type="text/scss" lang="scss" scoped>
  @import "../../assets/styles/color";
  /deep/ .van-cell__value{
    line-height: 33px;
  }
  /deep/ .van-cell__right-icon{
    line-height: 36px;
  }

  .payment-pre-deposit {
    margin-top: 20px;
    padding: 10px 15px;

    .van-checkbox {
      display: flex;
    }
    .van-password-input {
      margin-top: 10px;
    }
    .tip {
      padding-left: 15px;
      margin-top: 10px;
      color: #ffcc00;
      span {
        vertical-align: middle;
      }
      i {
        vertical-align: middle;
      }
      a {
        display: inline-block;
        background-color: #F59C1A;
        border-color: #F59C1A;
        color: #FFF;
        padding: 4px 8px;
        border-radius: 5px;
        margin: 10px 0 0 5px;
      }
    }
  }

  .success-item {
    width: 100%;
    height: 100%;
  }
  .inner-delivery {
    width: 170px;
    margin: 35px auto 20px;
    height: 50px;
    img {
      float: left;
      width: 50px;
      height: 50px;
    }
  }
  .content-delivery {
    display: flex;
    flex-direction: column;
    position: relative;
    height: 50px;
    justify-content: center;
  }
  .btns-delivery {
    display: flex;
    justify-content: space-around;
    width: 100%;
    .btns-item {
      width: 35%;
      height: 25px;
      border: 1px solid #d0d0d0;
      line-height: 20px;
      background-color: #fff;
      color: #333;
      font-size: 12px;
      text-align: center;
      border-radius: 5px;
    }
    button {
      background: none;
      border: none;
      line-height: 27px;
    }
  }
  /deep/ {
    .img-payment {
      width: 150px;
      height: 35px;
    }
  }
</style>
