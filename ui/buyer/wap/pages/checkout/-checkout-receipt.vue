  ...
  export default {
    ...
    methods: {
      ...
      async handleToggleType(receipt) {
        ...
        // 选定发票内容 单独：：：： 如果选定的是电子普通发票，则判断默认选择的是那个
        if (this.current_receipt_type === 'ELECTRO') {
          const default_receipt = this.receipts.find(item => item.is_default)
          this.receipt_contents.forEach(key => key.code = false)
          if (default_receipt) {
            const current_receipt = this.receipt_contents.find(key => key.receipt_content === default_receipt.receipt_content)
        ...
        if (this.current_receipt_type === 'VATOSPECIAL') {
          ...
          if (this.receipt) {
            this.receiptForm.detail_addr = this.receipt.detail_addr || this.invoiceAddressInfo.detail_addr || ''
            this.receiptForm.member_mobile = this.receipt.member_mobile || this.invoiceAddressInfo.member_mobile || ''
            this.receiptForm.member_name = this.receipt.member_name || this.invoiceAddressInfo.member_name || ''
            this.regions = [
              this.receipt.province_id || this.invoiceAddressInfo.province_id,
              this.receipt.city_id || this.invoiceAddressInfo.city_id,
              this.receipt.county_id || this.invoiceAddressInfo.county_id,
              this.receipt.town_id || this.invoiceAddressInfo.town_id
            ]
            this.receiptForm.address = `${this.receipt.province || this.invoiceAddressInfo.province || ''} ${this.receipt.city || this.invoiceAddressInfo.city || ''} ${this.receipt.county || this.invoiceAddressInfo.county || ''} ${this.receipt.town || this.invoiceAddressInfo.town || ''}`
          }
