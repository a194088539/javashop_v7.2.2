<template>
  <div id="checkout" style="background-color: #f7f7f7">
    ...
    <div v-else class="checkout-container">
      <!--收货地址 start-->
      <van-cell-group>
        <van-cell is-link :to="{ path: '/member/shipping-address', query: { from: 'checkout' } }">
      ...
      <van-cell-group>
        ...
        <!--支付配送 end-->
        <!--发票信息 start-->
        <van-cell title="发票信息" is-link @click="handleShowReceiptPopup" class="remark-cell">
          <span v-if="!params.receipt || !params.receipt.receipt_title || !orderTotal.original_price">不开发票</span>
          <span v-else>{{ params.receipt.receipt_type | receiptType }}-{{ params.receipt.receipt_title }}-{{ params.receipt.receipt_content }}</span>
        </van-cell>
        <!--发票信息 end-->
      </van-cell-group>
      <van-cell-group>
        <!--优惠券 start-->
        <van-cell v-if="!isAssemble" is-link @click="showCouponsPopup = true">
          <div slot="title">
            <span>优惠券</span>
      ...
      <van-cell-group class="price-cells">
        ...
        <van-cell title="积分抵扣" v-if="orderTotal.exchange_point !== '0'" class="exchange-cell">
          <span>-{{ orderTotal.exchange_point }}积分</span>
  ...
  export default {
    ...
    async mounted() {
      ...
      if (this.isAssemble) {
        ...
        await API_Trade.setPaymentType(this.params.payment_type)
      }
    ...
    methods: {
      /** 发票信息发生改变 */
      async handleReceiptChanged(receipt) {
        if (receipt.receipt_type === 'VATOSPECIAL') {
          ...
          this.$set(this.params, 'receipt', params.receipt)
          this.loading = false
          return
        }
      ...
      handleShowReceiptPopup(){
        if(!this.orderTotal.original_price){
      ...
      handleCreateTrade() {
        /** 先调用创建订单接口，再跳转到收银台 */
        Promise.all([
          this.isAssemble ? API_Trade.createAssembleTrade('WAP', this.$route.query.order_id) : API_Trade.createTrade(this.way),
        ]).then(response => {
      ...
      GET_Inventories() {
        Promise.all([
          this.isAssemble ? API_Trade.getAssembleCarts() : API_Trade.getCarts('checked', this.way),
        ]).then(response => {
          // console.log(5555,response);
          const { cart_list, coupon_list, total_price } = response[0]
          this.orderTotal = total_price
          // console.log(5555,this.orderTotal);
          let coupon_num = 0
          coupon_num += coupon_list ? coupon_list.filter(coupon => coupon.enable !== 0).length : 0
