      ...
      handleOnSubmit(_keyword) {
        ...
        if (searchType === '商品') {
          ...
          if (this.isMiniPrograme) {
            if(keyword) {
              wx.miniProgram.navigateTo({ url: `/pages/search/search?keyword=${keyword}` })
            } else {
              wx.miniProgram.navigateTo({ url: '/pages/search/search' })
            }
          } else {
            this.$router.push({path:'/goods',query:{keyword:keyword}});
            // window.location.href = keyword ? `/goods?keyword=${keyword}` : '/goods'
          }
        } else {
          if (this.isMiniPrograme) {
            wx.miniProgram.navigateTo({ url: `/pages/shop/shop?keyword=${keyword}` })
          } else {
            this.$router.push({path:'/shop',query:{keyword:keyword}});
