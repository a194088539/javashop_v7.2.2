...
export function getCarts(show_type = 'all', way = 'CART') {
  return request({
    url: `trade/carts/${show_type}?way=`+way,
    method: Method.GET,
    needToken: true,
    loading: false
  })
 ...
 * 创建订单
 */
export function createTrade(client = 'PC', way) {
  return request({
    url: 'trade/create',
    method: Method.POST,
    needToken: true,
 ...
 * @param coupon_id
 */
export function useCoupon(seller_id, mc_id, way) {
  return request({
    url: `trade/promotion/${seller_id}/seller/${mc_id}/coupon`,
    method: Method.POST,
    needToken: true,
    params: {
      way
