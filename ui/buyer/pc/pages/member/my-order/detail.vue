<template>
  <div id="order-detail">
        ...
        <div class="info-list">
          ...
          <template v-if="order.receipt_history && order.receipt_history.receipt_title">
            ...
            <dl>
              <dt>发票详情:</dt>
              <dd>
                <nuxt-link :to="'/member/my-receipt/detail?history_id=' + order.receipt_history.history_id">查看详情</nuxt-link>
              </dd>
            </dl>
          </template>
          ...
          <dl><dt>商品总价：</dt><dd>￥{{ (order.goods_original_total_price || 0) | unitPrice }}</dd></dl>
          ...
          <dl><dt>订单总价：</dt><dd style="font-size: 16px; color: #49afcd">￥{{ (order.order_price || 0) | unitPrice }}</dd></dl>
          <dl><dt>在线支付金额：</dt><dd style="font-size: 22px;" class="price">￥{{ (order.need_pay_money || 0) | unitPrice }}</dd></dl>
          <dl><dt>预存款支付金额：</dt><dd style="font-size: 22px;" class="price">￥{{ (order.balance || 0) | unitPrice }}</dd></dl>
        </div>
      </div>
      <div class="status-order">
        <h2>订单状态：{{ order.order_status_text }}
        ...
        <ul class="status-list">
          ...
          <li v-if="order.order_operate_allowable_vo.allow_check_cancel && order.order_status != 'CANCELLED'">
            - 订单申请取消中，您可以进入
            <strong><a :href="'/member/service-record'" target="_blank">售后管理</a></strong>
            中查看取消进度
          </li>
          ...
          <li v-if="order.order_operate_allowable_vo.allow_pay">
            - 如果您尚未对该订单进行支付，请
            <strong><nuxt-link :to="'/checkout/cashier?order_sn=' + order.sn">按此为订单付款</nuxt-link></strong>
            以确保商家及时发货。
          </li>
          ...
          <li v-if="order.order_status == 'PAID_OFF' && order.order_operate_allowable_vo.allow_service_cancel">
            - 当前订单已经付款成功，正在等待商家发货
          </li>
    ...
    <div v-if="skuList" class="goods-list">
      <sku-list :skuList="skuList" :order_type="order_type" name="name" price="purchase_price" total="subtotal"></sku-list>
    </div>
    ...
    methods: {
      ...
      GET_OrderDetail() {
        API_Order.getOrderDetail(this.order_sn).then(response => {
          ...
          this.skuList = JSONbigString.parse(response.items_json)
