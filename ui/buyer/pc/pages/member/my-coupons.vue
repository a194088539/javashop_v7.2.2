        ...
        <li v-for="(coupon, index) in coupons.data" :key="index" class="coupon-item" :class="[coupon.used_status === 1 && 'used', coupon.used_status === 2 && 'overdue']">
          <div class="c-type">
            ...
            <div class="c-shop" v-else>
              仅限 <nuxt-link :to="'/shop/' + coupon.seller_id" style="color: #fff">{{ coupon.seller_name }} 店铺使用</nuxt-link>
            </div>
            <div class="c-time">
          ...
          <div class="c-othr">
            <p v-if="coupon.order_sn">订单号：{{coupon.order_sn}}</p>
            <p v-if="coupon.used_status === 1">已使用</p>
            <p v-else-if="coupon.used_status === 2">已过期</p>
            <div v-else>
              <nuxt-link v-if="coupon.use_scope === 'ALL'" :to="'/'" class="use-btn">立即使用</nuxt-link>
              <nuxt-link v-else-if="coupon.use_scope === 'CATEGORY'" :to="'/goods?category=' + coupon.scope_id" class="use-btn">立即使用</nuxt-link>
              <nuxt-link v-else-if="coupon.use_scope === 'SOME_GOODS'" :to="'/goods/' + coupon.scope_id" class="use-btn">立即使用</nuxt-link>
    ...
    data() {
      return {
        ...
        tabs: [
          ...
          {label: '已用优惠券', key: 2, active: false},
          {label: '已过期优惠券', key: 3, active: false}
        ]
      }
    },
    mounted() {
      ...
      GET_Coupons() {
        API_Members.getCoupons({ ...this.params, status: this.tabs.find(item => item.active).key }).then(response => {
          ...
          for( let i in this.coupons.data){
            ...
            // 如果返回的use_scope为部分商品 SOME_GOODS, 通过店铺sku 获取商品详情id
            if( this.coupons.data[i].use_scope === 'SOME_GOODS' ){ 
              API_Goods.getGoodsSkusShop(this.coupons.data[i].scope_id).then(res => {
                this.coupons.data[i].scope_id = res
              })
            }
          }
