<template>
  <div id="member-index">
    <div class="user-title">
      <div class="user-item">
        <nuxt-link to="/member/my-profile" class="user-avatar"  v-if="user.face && user.face !== 'null'">
          <en-face :url="user.face"/>
        </nuxt-link>
        <nuxt-link to="/member/my-profile" class="user-avatar" v-else>
      ...
      <nuxt-link to="/member/goods-collection" class="other-item">
        <p>收藏的商品</p>
        <i class="iconfont ea-icon-collection-of-goods"></i>
        <p class="num">{{ statisticsNum.goods_collect_count || 0}}</p>
      </nuxt-link>
      <nuxt-link to="/member/shop-collection?type=shop" class="other-item">
        <p>收藏的店铺</p>
    ...
    <div class="box-item">
            ...
            <template v-for="(order, index) in orderData.data">
              <div v-if="index < 3" :key="order.sn" class="order-item" >
                ...
                <div class="order-info">
                  <a :href="'/goods/' + order.sku_list[0].goods_id" class="goods-name" target="_blank" v-html="order.sku_list[0].name">{{ order.sku_list[0].name }}</a>
      ...
      <div class="item right cart">
        <div class="item-title">
          <h2>浏览记录</h2>
          <nuxt-link to="/member/my-footprint">查看全部 	&gt;&gt;</nuxt-link>
        </div>
        <div class="item-content">
          ...
          <template v-else>
            ...
            <div v-for="(foot, index) in footPrints" :key="index">
              ...
              <div v-if="_index < 4" :key="goods.goods_id" class="cart-item">
                <nuxt-link target="_blank" :to="`/goods/${goods.goods_id}`" class="goods-image">
                  <img :src="goods.goods_img" :alt="goods.goods_name"/>
                </nuxt-link>
                <div class="goods-name">
                  <nuxt-link target="_blank" :to="`/goods/${goods.goods_id}`" v-html="goods.goods_name">{{ goods.goods_name }}</nuxt-link>
                  <p><em>￥{{ goods.goods_price | unitPrice }}</em></p>
                </div>
    ...
    <div class="box-item">
      <div class="item left goods-collection">
        <div class="item-title">
          <h2>商品收藏</h2>
          <nuxt-link to="/member/goods-collection">查看全部 	&gt;&gt;</nuxt-link>
        </div>
        <div class="item-content">
          ...
          <template v-else>
            <template v-for="(item, index) in goodsCollectionData.data">
              ...
              >
                <a :href="'/goods/' + item.goods_id" target="_blank">
      ...
      <div class="item right shop-collection">
        <div class="item-title">
          <h2>店铺收藏</h2>
          <nuxt-link to="/member/shop-collection?type=shop">查看全部 	&gt;&gt;</nuxt-link>
        </div>
        <div class="item-content">
