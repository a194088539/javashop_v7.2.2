    ...
    <div class="member-nav" v-if="$route.path !== '/member/my-comments/detail'">
      <ul class="member-nav-list">
        <li v-for="item in navList" :key="item.status">
	        <nuxt-link v-if="item.status === 'WAIT_COMMENT'" :to="'/member/my-comments'">{{ item.title }}</nuxt-link>
          <nuxt-link v-else :to="'/member/my-comments?comments_status=' + item.status">{{ item.title }}</nuxt-link>
        </li>
      </ul>
    </div>
