    ...
    <template v-else>
      ...
      <div v-if="params" class="ckt-control">
        ...
        <div class="ckt-content">
          ...
          <!--发票信息 start-->
          <checkout-receipt :getInventories="GET_Inventories" :receipt="params.receipt" @change="(receipt) => { params.receipt = receipt }"/>
          <!--发票信息 end-->
        </div>
        <div class="ckt-total">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /** 收货地址发生改变 */
      handleAddressChanged(address, type) {
        this.selectedAddress = address;
        this.params.address_id = address.addr_id;
        if (type) {
          this.params.payment_type = type
        }
        this.GET_Inventories()
      },
      /** 格式化地址信息 */
      ...
      handleCreateTrade() {
        /** 先调用创建订单接口，再跳转到收银台 */
        API_Trade.createTrade('PC', this.way).then(response => {
      ...
      /** 获取购物清单 */
      GET_Inventories() {
        return API_Trade.getCarts('checked',this.way).then(response => {
          this.inventoryList = response.cart_list;
          this.couponsList = response.coupon_list
