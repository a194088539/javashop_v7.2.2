<template>
  <div>
    ...
    <el-dialog
      ...
      <el-form :model="paymentForm" label-position="top" :rules="paymentRules" ref="paymentForm">
...
<script>
  ...
  export default {
    ...
    methods: {
      uploadSuccess(res) {
        this.$set(this.paymentForm, 'image', res.url)
      },
      /** 图片移除之后 */
      delGiftImg() {
        this.fileList = []
      ...
      handleEditPayment(index, row) {
        API_Payment.getPaymentDetail(row.plugin_id).then(response => {
          ...
          this.fileList = this.paymentForm.image ? [{ name: 'payment_image', url: this.paymentForm.image }] : []
