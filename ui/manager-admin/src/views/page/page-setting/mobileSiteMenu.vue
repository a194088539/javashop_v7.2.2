  ...
  export default {
    ...
    data() {
      return {
        ...
        siteMenuRules: {
          ...
          image: [
            this.MixinRequired('请选择导航图片！'),
            {
              validator: (rule, value, callback) => {
                if (!this.siteMenuForm.image) {
                  callback(new Error('请选择导航图片！'))
                } else {
                  callback()
                }
              },
    ...
    methods: {
      ...
      handleUploadSuccess(res) {
        this.siteMenuForm.image = res.url
        this.navImage = [{ name: 'image', url: res.url }]
      ...
      handleEditSiteMenu(index, row) {
        ...
        this.navImage = [{ name: 'image', url: row.image }]
        this.dialogVisible = true
      },
      ...
      handleDeleteSiteMenu(index, row) {
        this.$confirm('确定要删除这个导航菜单吗？', '提示', { type: 'warning' }).then(() => {
          API_SiteMenu.deleteSiteMenu(row.navigation_id).then(() => {
