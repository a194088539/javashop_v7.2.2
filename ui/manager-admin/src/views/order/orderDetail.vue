        ...
        <div class="d-content">
          <div v-for="item in col.items" :key="item.key" class="item">
            <div v-if="item.label === '发票详情' && item.value !== '无' && item.value !== '不需要发票'">
              ...
              <span class="item-value"><a href="#" @click="toReceiptDetail(item.value)">查看详情</a></span>
            </div>
            <div v-else>
              <span class="item-label" v-html="item.label"></span>
              <span class="item-value" v-if="item.label === '联系方式'">{{ item.value | secrecyMobile }}</span>
      ...
      <el-col :span="24">
        ...
        <el-table :data="productList" :header-cell-style="{textAlign: 'center'}">
          ...
          <el-table-column label="商品名称" align="center">
            <template slot-scope="scope">
              <a :href="MixinBuyerDomain + '/goods/' + scope.row.goods_id" class="goods-name" target="_blank" v-html="scope.row.name">{{ scope.row.name }}</a>
          ...
          [
            {
              ...
              items: [
                ...
                { label: '在线支付金额', value: '￥' + f.formatPrice(o.need_pay_money) },
                { label: '预存款支付金额', value: '￥' + f.formatPrice(o.balance) },
                { label: '商品总金额', value: '￥' + f.formatPrice(o.goods_original_total_price) },
                ...promotions,
          ...
          [
            ...
            },
            {
              ...
              items: [
                ...
                { label: '发货时间', value: o.ship_time !== '0' ? f.unixToDate(o.ship_time) : '未发货' },
