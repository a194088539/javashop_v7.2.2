...
export default class PinTuanFlatList extends PureComponent {
  constructor(props) {
    super(props);
    this.params = {
      page_no: 1,
  ...
  _getGoodsList = async () => {
    ...
    try {
      const data = await API_Promotions.getPinTuanGoods(params);
      this.setState({
        loading: false,
      ...
      <TouchableOpacity
        onPress={() =>
          navigate('Goods', {
            id: item.goods_id,
            sku_id: item.sku_id,
            pintuan_id: item.pin_tuan_id,
          })
        }
        style={styles.item}>
        <Image source={{uri: item.thumbnail}} style={styles.item_image} />
        <View style={styles.item_detail}>
          ...
          <View style={styles.item_price}>
            <View style={styles.item_price_left}>
              <Price advanced price={item.sales_price} />
              <Text style={styles.item_price_orig}>
                ￥{Foundation.formatPrice(item.origin_price)}
              </Text>
            </View>
            <View style={styles.item_price_right}>
              <Text style={styles.item_btn}>去开团</Text>
              <Text style={styles.item_buy_num}>
                已出售：{item.buy_count}件
              </Text>
            </View>
          </View>
        </View>
